 ...
 */
function splitChunkIntoWords(chunk) {
    const words = chunk.text.trim().split(/\s+/).filter(Boolean);
    if (words.length === 0)
        return [];
    if (words.length === 1)
        return [{ text: words[0], timestamp: chunk.timestamp }];
...
async function generateCaptions(audioFilePath) {
    ...
    for (let i = 0; i < samples.length; i++)
        boostedSamples[i] = Math.max(-1, Math.min(1, samples[i] * 2.0));
